...
class SportecData:
    ...
    def load_tracking_data(
        ...
        team_mapping = lineup[["team_id", "home_away"]].drop_duplicates().set_index("team_id")["home_away"]
        tracking_df["ball_owning_team_id"] = tracking_df["ball_owning_team_id"].map(team_mapping)
...
if __name__ == "__main__":
    tracking_files = os.listdir("data/sportec/tracking")
    match_ids = [f.split(".")[0].split("-")[-1] for f in tracking_files]

    parquet_dir = "data/sportec/tracking_parquet"
    os.makedirs(parquet_dir, exist_ok=True)

    for match_id in match_ids:
        print(match_id)
        match = SportecData(match_id)
        match.tracking.to_parquet(f"{parquet_dir}/{match_id}.parquet")
        print()
